...
namespace Digia.Qt4ProjectLib
{
    ...
    public class QtProject
    {
        ...
        public void AddUic4BuildStep(VCFile file)
        {
            ...
            {
                throw new Qt4VSException(SR.GetString("QtProject_CannotAddUicStep", file.FullPath));
            }
        }
            ...
            try 
            {
                ...
                if (defaultFilter == null) 
                {
                    if (!vcPro.CanAddFilter(filter.Name)) 
                    {
                        ...
                        if (defaultFilter == null)
                            throw new Qt4VSException(SR.GetString("QtProject_CannotAddFilter", filter.Name));
                    }
                    else
                    {
                ...
                VCFilter targetFitler = defaultFilter;
                if (configFilter != null)
                    targetFitler = configFilter;
                else if (plaformFilter != null)
                    targetFitler = plaformFilter;

                if (targetFitler.CanAddFile(fileName))
                    return (VCFile)(targetFitler.AddFile(fileName));
                else
                    throw new Qt4VSException(SR.GetString("QtProject_CannotAddFile", fileName));
            }
            catch 
            {
                throw new Qt4VSException(SR.GetString("QtProject_CannotAddFile", fileName));
        ...
        public VCFilter FindFilterFromName(string filtername)
        {
            ...
            {
                throw new Qt4VSException(SR.GetString("QtProject_CannotFindFilter"));
            }
        }
        ...
        public VCFilter AddFilterToProject(FakeFilter filter)
        {
            try 
            {
                ...
                if (vfilt == null)
                {
                    if (!vcPro.CanAddFilter(filter.Name)) 
                    {
                        vfilt = FindFilterFromName(filter.Name);
                        if (vfilt == null)
                            throw new Qt4VSException(SR.GetString("QtProject_ProjectCannotAddFilter", filter.Name));
                    }
                    else
                    {
            ...
            {
                throw new Qt4VSException(SR.GetString("QtProject_ProjectCannotAddResourceFilter"));
            }
        }
        ...
        public void AddDirectories()
        {
            ...
            {
                throw new Qt4VSException(SR.GetString("QtProject_CannotCreateResourceDir"));
            }
        ...
        public void SetQtEnvironment(string qtVersion, string solutionConfig)
        {
            ...
            try
            {
                HelperFunctions.SetQtEnvironment(envPro, qtDir, solutionConfig);
            }
            catch (Exception)
            {
